//! # ra8875 driver
//! A async driver for the RA8875 display driver chip.
//!
//! Based on the code in https://github.com/ReeceStevens/ra8875 which in turn was
//! adapted from Adafruit's open-source driver for their RA8875 line of TFT displays.
...
impl<SPI, DLY> RA8875<SPI, DLY>
where
    SPI: SpiDevice,
    DLY: DelayNs,
{
    pub fn new(spi: SPI, delay: DLY, dimensions: (u32, u32)) -> Self {
        RA8875 {
            spi,
            dimensions,
            text_settings: TextModeSettings {
    ...
    fn write_data(&mut self, data: u8) -> Result<(), <SPI as ErrorType>::Error> {
        let mut buf: [u8; 2] = [0; 2];
        buf[0] = Command::DataWrite as u8;
        buf[1] = data;

        self.spi.transaction(&mut [Operation::Write(&buf)])?;
        Ok(())
    }
    ...
    fn read_data(&mut self) -> Result<u8, <SPI as ErrorType>::Error> {
        ...
        self.spi.transaction(&mut [
            Operation::Write(&[Command::DataRead as u8]),
    ...
    fn write_command(&mut self, command: u8) -> Result<(), <SPI as ErrorType>::Error> {
        let buf: [u8; 2] = [Command::CmdWrite as u8, command];
        self.spi.transaction(&mut [Operation::Write(&buf)])?;
        Ok(())
    ...
    fn write_register(
        ...
        self.write_command(register as u8)?;
        self.write_data(data)?;
        Ok(())
    }

    fn read_register(&mut self, register: Register) -> Result<u8, <SPI as ErrorType>::Error> {
        self.write_command(register as u8)?;
        let data = self.read_data()?;
        Ok(data)
    }

    pub fn self_check(&mut self) -> Result<u8, <SPI as ErrorType>::Error> {
        let data = self.read_register(Register::SelfTest)?;
        Ok(data)
    }

    pub fn set_up_pll(&mut self) -> Result<(), <SPI as ErrorType>::Error> {
        match self.dimensions {
            (480, 80) | (480, 128) | (480, 272) => {
        ...
        // NOTE: Delay time taken from the Adafruit library
    ...
    pub fn init(&mut self) -> Result<(), <SPI as ErrorType>::Error> {
        ...
        // Set up for 8 bit MCU interface, color depth 16 bt.
        self.write_register(Register::Sysr, cmds::Sysr::BBP_16 as u8)?;
        let t = match self.dimensions {
            (480, 272) => Timing {
    ...
    pub fn display_on(&mut self, on: bool) -> Result<(), <SPI as ErrorType>::Error> {
        if on {
            self.write_register(
                Register::Pwrr,
    ...
    pub fn gpiox(&mut self, on: bool) -> Result<(), <SPI as ErrorType>::Error> {
        if on {
    ...
    pub fn pwm1_out(&mut self, pulse: u8) -> Result<(), <SPI as ErrorType>::Error> {
        self.write_register(Register::P1dcr, pulse)
    }

    pub fn pwm1_config(&mut self, on: bool, clock: u8) -> Result<(), <SPI as ErrorType>::Error> {
        if on {
    ...
    pub fn pwm2_out(&mut self, pulse: u8) -> Result<(), <SPI as ErrorType>::Error> {
        self.write_register(Register::P2dcr, pulse)
    }

    pub fn pwm2_config(&mut self, on: bool, clock: u8) -> Result<(), <SPI as ErrorType>::Error> {
        if on {
    ...
    pub fn set_text_scale(&mut self, scale: u8) -> Result<(), <SPI as ErrorType>::Error> {
        ...
        tmp &= !(0xF);
        self.write_data(tmp | bit_pattern)?;
    ...
    pub fn graphics_mode(&mut self) -> Result<(), <SPI as ErrorType>::Error> {
        match self.mode {
            Mode::Graphics => Ok(()),
            Mode::Text => {
                let tmp = self.read_register(Register::Mwcr0)?;
    ...
    pub fn push_pixels(
        ...
        self.spi
            .transaction(&mut [Operation::Write(&[Command::DataWrite as u8])])?;

        let color_bytes = u16::to_be_bytes(color);

        for bytes in repeat(color_bytes).take(num_pixels as usize) {
            self.spi.transaction(&mut [Operation::Write(&bytes)])?;
        }
    ...
    pub fn set_cursor(&mut self, new_position: Coord) -> Result<(), <SPI as ErrorType>::Error> {
    ...
    fn fill_rect(&mut self) -> Result<(), <SPI as ErrorType>::Error> {
        self.write_command(Register::Dcr as u8)?;
        self.write_data(cmds::Dcr::DRAWSQUARE as u8)?;
        self.write_data(
    ...
    pub fn draw_point(
        ...
        let color_bytes: [u8; 2] = color.to_be_bytes();

        self.spi.transaction(&mut [
            Operation::Write(&[Command::DataWrite as u8]),
            Operation::Write(&color_bytes),
        ])?;
        // self.cs.set_low().ok().unwrap();
        // self.spi_send(Command::DataWrite as u8)?;
        // self.spi_send((color >> 8) as u8)?;
    ...
    pub fn fill_screen(&mut self, color: u16) -> Result<(), <SPI as ErrorType>::Error> {
        let (width, height) = self.dimensions;
...
impl<SPI, DLY> Write for RA8875<SPI, DLY>
where
    SPI: SpiDevice,
    DLY: DelayNs,
{
...
impl<SPI, DLY> OriginDimensions for RA8875<SPI, DLY>
where
    SPI: SpiDevice,
    DLY: DelayNs,
{
    fn size(&self) -> Size {
        Size::new(self.dimensions.0 as u32, self.dimensions.1 as u32)
    }
}

impl<SPI, DLY> DrawTarget for RA8875<SPI, DLY>
where
    SPI: SpiDevice,
    DLY: DelayNs,
{
        ...
        I: IntoIterator<Item = Pixel<Self::Color>>,
    {
        let bounding_box = primitives::Rectangle::new(
            Point::new(0, 0),
            Size::new(self.dimensions.0, self.dimensions.1),
        );
        ...
        I: IntoIterator<Item = Self::Color>,
    {
        ...
        for (point, color) in point_color_pairs {
            if Some(point.y) != last_y {
                ...
                self.write_command(Register::Mrwc as u8)?;
                self.spi.write(&mut [Command::DataWrite as u8])?;
            }
            let storage_color = color.into_storage();
            self.draw_point(to_coord(point), color.into_storage())?;

            let mut storage_color_bytes = u16::to_be_bytes(storage_color);

            self.spi.write(&mut storage_color_bytes)?;
...
// Removing this as all errors are SPI errors
// #[derive(Clone, Copy, Debug)]
// pub enum <SPI as ErrorType>::Error<E> {
//     /// An error reading data over SPI
//     SpiRead(E),
//     /// An error writing data over SPI
//     SpiWrite(E),
// }

// impl From<embedded_hal_async::spi::ErrorType::Error>
//     for DisplayError<embedded_hal_async::spi::ErrorType::Error>
// {
//     fn from(value: embedded_hal_async::spi::ErrorType::Error) -> Self {
//         <SPI as ErrorType>::Error::
//     }
// }
